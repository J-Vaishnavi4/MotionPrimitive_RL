...
class turtlebot3_burger_GymEnv_forward(gym.Env):
  ...
  def reset(self,seed = None):
    ...
    self._initial_orientation = self._observation[3]
    self._observation[0]=0  # initial displacement = 0
    self._observation[3]=0  # initial yaw_change = 0
    # self.reward_value=0
  ...
  def step(self, action):
    ...
    # self._observation.pop()
    self._observation[3] = yaw_change
  ...
  def _termination(self):
    ...
    return lateral_deviation > 0.03 or abs(yaw_change) > 0.3
  ...
  def _reward(self, action):
    ...
    lateral_deviation = abs(displacement*math.sin(alpha))
    rew1 = 5*action[0]
    rew2 = -1000*lateral_deviation -50*abs(yaw_change)
