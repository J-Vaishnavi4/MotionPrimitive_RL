...
class prediction_model:
    def __init__(self, 
        ...
        self.list_of_models = []
        # list_of_models, compensation_time = self.predict_model()
        # return list_of_models, compensation_time

    def linear_time_predict(self, model_num_lin, out):
        ...
        for i in range(start, model_num_lin.shape[0], stride):
            out[i], lin_std_prediction = pickle.load(open("./"+self.case+"/GP_models/"+self.MP_name+str(model_num_lin)+"_noise.dump", "rb")).predict(np.array([self.required_displacement]).reshape(1,-1), return_std=True)
            lin_predict_time = round(out[i][0][0])
    ...
    def predict_model(self):
        ...
        if self.required_displacement>0 and len(lin_time_list)==0:
            ...
            out = np.empty_like(lin_model_nums)
            threads_per_block = 128
            blocks_per_grid = 30
            start_time_lin = time.time()
            self.linear_time_predict[blocks_per_grid, threads_per_block](lin_model_nums, out)
