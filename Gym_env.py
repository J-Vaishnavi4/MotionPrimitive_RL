...
class Gym_environment(gym.Env):
  ...
  def reset(self,seed = None):
    ...
    self._observation[1]=0  # initial ld / yaw_change = 0
    if self._motion == "CW" or self._motion=="CCW":
  ...
  def reset_MP(self, motion = "F", init_angle = "init_angle", init_position="init_position"):
    self._initial_orientation = init_angle
    self._robot_initial_pos = init_position
    self._motion = motion
    self._observation = self._robot.getObservation()
    self._robot_initial_pos = self._observation[0]
    self._initial_orientation = self._observation[1]
  ...
  def step(self, action):
    ...
    for i in range(self._actionRepeat):
      ...
      if self._motion == "F" or self._motion == "B":
        self.robot_observations_translation()
        done = self._termination_translation()
      else:
        self.robot_observations_rotation()
        done = self._termination_rotation()
        lateral_deviation = self.displacement
      if done:
        break
    ...
    info['x'] = self.robot_pos[0]
    info['y'] = self.robot_pos[1]
  ...
  def robot_observations_translation(self,):
    ...
    # print( "linV_world: ", linVw, "norms: ", np.linalg.norm(linVw))
  ...
  def robot_observations_rotation(self,):
    ...
    d = (abs(np.asarray(robot_pos) - np.asarray(self._robot_initial_pos)))
    self.displacement = math.sqrt(math.pow(d[0],2) + math.pow(d[1],2))
  def _termination_rotation(self):

    return self.displacement > 0.01 or self.yaw_change > 4
  ...
  def CW_reward(self, action):
    ...
    if angVel <= 1*w_max:
      ...
      rew1 = 0
    rew2 = -40*self.displacement
    return rew1, rew2, #self.yaw_change, self.displacement, self.displacement, self.robot_pos
