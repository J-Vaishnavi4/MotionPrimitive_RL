...
class turtlebot3_burger_GymEnv_CW(gym.Env):
  ...
  def reset(self,seed = None):
    ...
    self._initial_orientation = self._observation[0] #-self._observation[0]*(self._observation[0] <= 0)+(-self._observation[0] + 2*math.pi)*(0<self._observation[0]<=math.pi)
    info = {}
  ...
  def _termination(self):
    ...
    # if aV[2]>0:
    #     print(self._p.getEulerFromQuaternion(robot_orn)[2])
  ...
  def _reward(self):
    ...
    # print("prev data: ", yaw, self._initial_orientation)
    if 0<yaw<=math.pi:
      yaw = -yaw + 2*math.pi
    else:
      yaw = -yaw
    yaw_change = yaw - self._initial_orientation #- yaw
    lV, aV = self._p.getBaseVelocity(self._robot.robotUniqueId)

    # print("data: ",yaw, yaw_change, aV[2])
