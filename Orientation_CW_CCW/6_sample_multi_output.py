...
def main():
    ...
    model = ppo.PPO.load(os.path.join(currentdir,"./models/PPO/orientation_MP/"+MP_name+"13"))       #CW
    obs,info = env.reset()
    done = False
    rew=0
    with open("./Samples/"+MP_name+'/samples_13_multi_output.csv','w',newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Yaw_change", "time", "lin_vel", "ang_vel"])
    action = [0,0]
    data = obs[1]
    data = np.append(data, (0,0,0))
    
    with open("./Samples/"+MP_name+'/samples_13_multi_output.csv','a', newline='') as file:
        writer = csv.writer(file)
    ...
    for j in range(350):
        ...
        for i in range(j+2):
            ...
            if i == j+1:
                data = obs[1]
                data = np.append(data, (i,action[0],action[1]))
                with open("./Samples/"+MP_name+'/samples_13_multi_output.csv','a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(data)
                    file.close()
