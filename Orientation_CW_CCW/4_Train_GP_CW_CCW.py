...
with open("./GP_models/"+MP_name+"/no_noise.dump" , "wb") as f:
     pickle.dump(gaussian_process, f)

model1 = pickle.load(open("./GP_models/"+MP_name+"/no_noise.dump","rb"))
...
with open("./GP_models/"+MP_name+"/noisy.dump" , "wb") as f:
     pickle.dump(gaussian_process, f)

model2 = pickle.load(open("./GP_models/"+MP_name+"/noisy.dump","rb"))

"---------------PREDICTION------------"
required_yaw = 2.5  #radians
mean_prediction, std_prediction = model1.predict(np.array([required_yaw]).reshape(1, -1), return_std=True)
