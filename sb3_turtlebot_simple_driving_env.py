...
class SimpleDrivingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if (turtle_ob[0] >= 9 or turtle_ob[0] <= -9 or
            turtle_ob[1] >= 9 or turtle_ob[1] <= -9):
            reward = -50
            print("out of plane")
            self.done = True
    ...
    def reset(self, seed=None, options=None):
        ...
        self.prev_dist_to_goal = math.sqrt(((turtle_ob[0] - self.goal[0]) ** 2 +
                                           (turtle_ob[1] - self.goal[1]) ** 2))
        # self.prev_orientation = np.arctan(turtle_ob[3]/turtle_ob[2])
        self.initial_orientation = np.arctan(turtle_ob[3]/turtle_ob[2])
        self.prev_velocity = math.sqrt(((turtle_ob[4])**2)+(turtle_ob[5])**2)
