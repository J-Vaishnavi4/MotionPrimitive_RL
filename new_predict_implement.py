...
def predict_model(node_x_target, node_y_target, robot_x, robot_y, robot_angle, time_target):
    ...
    if required_yaw_change==0:
        ...
        MP_rotn = 0
        rotn_predict_time = 0
        if required_displacement>0 and len(lin_time_list)==0:
            MP_name = "F"
            MP_lin = 3
            
            for model_num_lin in [10,9,8,7,6,5,4,3,2]:
                start_load = time.time()
                GP_linear = pickle.load(open("./"+case+"/GP_models/"+MP_name+str(model_num_lin)+"_noise.dump", "rb"))
                print("load:", time.time()-start_load)
                ...
                if abs(total_predicted_time-time_req)<40:
                    break
        ...
        for model_num_rotn in rtn_model_num:
            ...
            if required_displacement>0 and len(lin_time_list)==0:
                ...
                for model_num_lin in [10,9,8,7,6,5,4,3,2]:
                    ...
                    total_predicted_time = rotn_predict_time + lin_predict_time
                    if abs(total_predicted_time-time_req)<30:
                        break
                ...
                closest_time_cal =  min(total_time_list, key=lambda x:abs(x-time_req))
                if closest_time_cal<30:
                    index = total_time_list.index(closest_time_cal)
                    lin_predict_time = lin_time_list[index]
                    model_num_lin = index+1
                    break
